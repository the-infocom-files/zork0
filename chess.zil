...
"\"The terrain is strange and unfamiliar; I am too terrified to move!\"" CR>)
		      ...
		      (T
		       ...
		       <PUTB ,PIECE-MOVE-TABLE ,DIR-CNT ,PRSO>
...
<ROUTINE MOVE-PIECE
	 ...
	 <SET CNT 0>
	 <REPEAT ()
		 <SET DIR <GETB ,PIECE-MOVE-TABLE .CNT>>
		 <COND (<EQUAL? .DIR <>>
			<RETURN>)>
...
"\"My word! There appears to be a wall in the way!\"" CR>)>
			<RETURN>)>
 		 <SET NEW-LOC <+ <* <- .NEW-RANK 1> 8> <- .NEW-FILE 1>>>
		 <COND (<AND <GETB ,PIECE-MOVE-TABLE .CNT>
			     <NOT <EQUAL? ,WINNER ,BLACK-KNIGHT ,WHITE-KNIGHT>>
...
"\"Alas, the path between here and there is not unobstructed.\"" CR>
			<RETURN>)>>
	 <SET DIR <DIR-TO-STRING <GETB ,PIECE-MOVE-TABLE 0>>>
...
<CONSTANT NE-EXITS ;"17 rooms"
	<PTABLE 12 13 14 20 22 27 28 29 33 36 41 43 46 49 50 53 54>>

<CONSTANT EAST-EXITS ;"15 rooms"
	<PTABLE 5 6 12 22 26 30 34 38 42 44 51 56 57 61 62>>
...
<ROUTINE STORE (OFFSET L "OPT" (RM ,HERE) "AUX" (CNT 0) F N OBJ)
	 <SET F <FIRST? .RM>>
	 <COND (<AND <ULTIMATELY-IN? ,ROOSTER .RM>
		     <NOT <FSET? <LOC ,ROOSTER> ,ACTORBIT>>
		     <EQUAL? <LOC ,ROOSTER> <LOC ,WORM>>
		     <FSET? ,WORM ,ANIMATEDBIT>
		     <FSET? ,ROOSTER ,ANIMATEDBIT>>
		<SETG ROOSTER-BURP T>
		<REMOVE ,WORM>)>
	 <COND (<AND <ULTIMATELY-IN? ,FOX .OBJ>
		     <NOT <FSET? <LOC ,FOX> ,ACTORBIT>>
		     <EQUAL? <LOC ,FOX> <LOC ,ROOSTER>>
		     <FSET? ,ROOSTER ,ANIMATEDBIT>
...
<ROUTINE UNSTORE (OFFSET L "OPT" (RM ,HERE)
		  "AUX" CNT (LEN </ ,STORAGE-TABLE-LENGTH 2>) (NUM <+ .L .OFFSET>))
	 <REPEAT ()
	   <COND (<SET CNT <INTBL? .NUM ,STORAGE-TABLE .LEN *204*>>
		  <MOVE <GET .CNT 1> .RM>
		  <PUT .CNT 1 0>
		  <PUT .CNT 0 0>)
		 (T
