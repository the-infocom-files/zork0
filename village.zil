...
<ROUTINE VILLAGE-SEGMENT-F ("OPT" (RARG <>))
...
perimeter fortifications. A drawbridge ">
		...
		<TELL
" moat to the southeast, and roads lead northeast, southwest, and northwest.">)
	       (<AND <EQUAL? .RARG ,M-BEG>
		     <VERB? DIG>
		     <PRSO? ,GROUND>
		     <PRSI? ,SHOVEL>
		     <EQUAL? ,JUMP-X ,STUMP-X>
...
<ROUTINE PACKAGE-F ()
	 <COND (<VERB? READ EXAMINE>
		<FONT 4>
		<TELL
"\"From: Belznork Gibblewitz|
       F. M. Homing Pigeon Co.|
       FrobozzCo Bldg, 193-E|
       Flatheadia, FRV-9179|
|
  To:  Eek Numblatz|
       Int'l Curios, Inc.|
       28 Volcano View Lane|
       Gurth City, GTH-3791\"" CR>
		<FONT 1>)>>
...
"The world blurs, then darkens. You blink" ,ELLIPSIS>
		...
		<TELL "put the pigeon on the perch." CR>)>>
...
<END-SEGMENT>
\
<BEGIN-SEGMENT 0> ;"else disk flip in middle of drawing FrobozzCo Bldg. Encyc. entry"
...
<ROUTINE OFFICES-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		...
		<SETG BEEN-IN-FR-UPPER-FLOORS T>)
	       (<EQUAL? .RARG ,M-LOOK>
		<TELL "You are on Floor " N ,FLOOR-NUMBER>
		<PRINT " of the FrobozzCo Building. The office">
		<TELL "s
...
<ROUTINE FR-OUTER-OFFICES-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		...
		<RFALSE>)
	       (<EQUAL? .RARG ,M-LOOK>
		<TELL "You are in an office on floor " N ,FLOOR-NUMBER>
		<PRINT " of the FrobozzCo Building. The office">
		<TELL " has a lovely ">
...
<ROUTINE FR-PENTHOUSE-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,VILLAGE-SEGMENT-PIC ,PICINF-TBL>
		<COND (<NOT <FSET? ,FR-PENTHOUSE ,TOUCHBIT>>
		       <SETG DO-J T>
		       <QUEUE I-JESTER 1>)>
		<RFALSE>)>>
