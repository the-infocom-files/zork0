...
<ROUTINE ORACLE-F ("OPT" (RARG <>))
	 <COND (<AND <EQUAL? .RARG ,M-ENTER>
		...
		;<SETG ORACLE-EXIT-NUMBER <- <TELL-RANDOM 5> 1>>
...
<ROUTINE ORACLE-SEGMENT-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,ORACLE-SEGMENT-PIC ,PICINF-TBL>
...
combat. The rusty strips of metal by their side may have once been weapons.")
      ...
      (THINGS IRON ( MINE IRON ORE) IRON-PS)
...
the mountain. A short distance below is another, smaller ledge.")
      ...
      (THINGS ROCKY ( ROCK SPIRE) SPIRE-F)
      (ACTION ORACLE-SEGMENT-F)>

<ROUTINE SPIRE-F ()
	 <COND (<AND <VERB? TIE>
		     <PRSO? ,SNAKE>
		     <NOT <FSET? ,SNAKE ,ANIMATEDBIT>>>
		<SETG ROPE-PLACED T>
		<MOVE ,SNAKE ,HERE>
		<TELL
...
a beautiful young maiden, in peaceful sleep. Then, the vision fades." CR>)
		      (<PRSO? ,WAND>
		       <PRINT "The reflection of ">
		       <TELL
"the wand is unnaturally still; more frozen than even
...
have dropped the ring." CR>)>
		      ...
		      (<PRSO? ,N-S-PASSAGE ,NW-SE-PASSAGE>
		       <PRINT "The reflection of ">
		       <TELL
"the passage reveals a feature which is invisible when
...
an other-wordly glow." CR>)
		      (<PRSO? ,PIGEON>
		       <PRINT "The reflection of ">
		       <TELL "the inert pigeon is most startling: it appears
...
powerful bird is flying toward the nest from a great distance." CR>)
		      (<PRSO? ,AMULET>
		       <PRINT "The reflection of ">
		       <TELL
"the amulet is suffused in a glow of amazing energies! A
...
<ROUTINE POEM-F ()
	 <COND (<VERB? READ EXAMINE>
		<TELL ;"tired pine, iron mine, wore, magic store"
...
A relic found in a _ _ _ _ _   _ _ _ _ _\"" CR>)>>
...
<ROUTINE NOTEBOOK-F ()
	 <COND (<VERB? READ OPEN EXAMINE>
		<COND (<NOT <ULTIMATELY-IN? ,NOTEBOOK>>
		       ;"else you can read it with goggles while it's in trunk"
		       <TELL ,YNH TR ,NOTEBOOK>
		       <RTRUE>)>
		<TELL
...
entrance, and a path runs off to the west.")
      ...
      (ACTION OUTSIDE-SHACK-F)>

<ROUTINE OUTSIDE-SHACK-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,ORACLE-SEGMENT-PIC ,PICINF-TBL>
		<RFALSE>)
	       (<AND <EQUAL? .RARG ,M-BEG>
...
hovel's entrance, to the east, is a faded sign.")
      ...
      (ACTION OUTSIDE-HOVEL-F)>

<ROUTINE OUTSIDE-HOVEL-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,ORACLE-SEGMENT-PIC ,PICINF-TBL>
		<RFALSE>)
	       (<AND <EQUAL? .RARG ,M-BEG>
...
signs, there's one next to the mine entrance.")
      ...
      (ACTION MINE-ENTRANCE-F)>

<ROUTINE MINE-ENTRANCE-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,ORACLE-SEGMENT-PIC ,PICINF-TBL>
		<RFALSE>)
	       (<AND <EQUAL? .RARG ,M-BEG>
...
path heads eastward.")
      ...
      (ACTION NORTH-OF-ANTHAR-F)>

<ROUTINE NORTH-OF-ANTHAR-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,ORACLE-SEGMENT-PIC ,PICINF-TBL>
		<RFALSE>)
	       (<AND <EQUAL? .RARG ,M-BEG>
	 ...
	 <COND (<EQUAL? .ARG ,M-WINNER>
		<COND (<FSET? ,BAT ,TRYTAKEBIT>
		       <COND (<IN? ,LARGE-VIAL ,LOCAL-GLOBALS>
			      ...
			      <TELL
"\"Bring us exactly six gloops of water from the Great Underground Oasis, and we shall
remove the enchantment on that which you seek.\" A vial appears in front of you." CR>)
			     (T
			      <TELL "\"No chatting until you fetch the ">
			      <COND (,VIAL-GIVEN
				     <TELL "earwax of brogmoid">)
				    (T
				     <TELL "six gloops of Oasis water">)>
			      <TELL "!\"" CR>)>)
		      (T
		       <TELL
...
fingers, missing your rump by inches as you scurry from the cave." CR CR>
		...
		<COND (<PRSI? ,WATER>
		       <COND (<OR <AND <PRSO? ,SMALL-VIAL>
			      ...
			      <TELL "The " D ,PRSO " is already">
			      <PRINT " filled to the brim">
			      <TELL "!" CR>)
			     ...
			     (<PRSO? ,SMALL-VIAL>
			      ...
			      <COND (<AND <NOUN-USED? ,WATER
				    ...
				    (T
				     ...
				     <PRINT "You fill the vial to the brim.">
				     <CRLF>)>)
			     (T ;"PRSO is large vial"
			      ...
			      <COND (<AND <NOUN-USED? ,WATER
				    ...
				    (T
			      	     ...
			      	     <PRINT "You fill the vial to the brim.">
				     <CRLF>)>)>)
		      (<EQUAL? .PRSI-VIAL-GLOOPS 0>
		       <TELL "The " D ,PRSI " is already empty!" CR>)
		      (<AND <PRSO? ,SMALL-VIAL>
			    <PRSI? ,LARGE-VIAL>>
		...
		<RTRUE>)
	       (<VERB? EMPTY>
		<COND (<EQUAL? .PRSO-VIAL-GLOOPS 0>
		       <TELL "The " D ,PRSO " is already empty!" CR>)
		      (<AND <PRSO? ,SMALL-VIAL>
			    <PRSI? ,LARGE-VIAL>>
		       <POUR-VIALS ,SMALL-VIAL ,LARGE-VIAL>)
		      (<AND <PRSI? ,SMALL-VIAL>
		...
		<COND (<G? ,LARGE-VIAL-GLOOPS .SPARE-ROOM>
		      ...
		      (T
		       ...
		       <COND (,LARGE-VIAL-IMPRECISE
			      <SETG LARGE-VIAL-IMPRECISE <>>
			      <SETG SMALL-VIAL-IMPRECISE T>)>
		       <TELL " The " D ,LARGE-VIAL>
		       <PRINT " has been completely emptied into">
		       <TELL TR ,SMALL-VIAL>)>)
...
<ROUTINE RUBBLE-ROOM-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		...
		<COND (<FSET? ,HARDHAT ,WORNBIT>
		       <TELL "Clunk! A bit of rubble bounces off your hardhat." CR>
