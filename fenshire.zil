...
<ROUTINE FENSHIRE-SEGMENT-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,FENSHIRE-SEGMENT-PIC ,PICINF-TBL>
...
The only exit is south.")
      ...
      (ACTION FENSHIRE-SEGMENT-F) ;"filled in by SLIDE-SHOW">

<ROUTINE SMALLER-HANGAR-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,FENSHIRE-SEGMENT-PIC ,PICINF-TBL>
		<COND (<NOT <FSET? ,SMALLER-HANGAR ,TOUCHBIT>>
		       <QUEUE I-FOX -1>)>
		<RFALSE>)>>
...
<ROUTINE DESERTED-CASTLE-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,FENSHIRE-SEGMENT-PIC ,PICINF-TBL>
		<COND (<NOT <FSET? ,DESERTED-CASTLE ,TOUCHBIT>>
		       <QUEUE I-ROOSTER -1>)>
		<RFALSE>)>>
...
<ROOM RUINED-HALL
      ...
      (THINGS FADED FRESCO FRESCO-F
	      CHOKED FIREPLACE FIREPLACE-F)>
...
former condition: decayed but passable! ">)
		...
		<TELL "A stairway once led upwards, but there's little left of it.">)
...
"You cut yourself to ribbons on the broken glass." CR>)
			     ...
			     (<VERB? EXAMINE>
			      <TELL
"Many of the window panes are shattered." CR>)
			     (<VERB? LOOK-INSIDE>
