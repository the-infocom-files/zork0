...
<ROUTINE FOOZLE-SEGMENT-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,FOOZLE-SEGMENT-PIC ,PICINF-TBL>
		<RFALSE>)>> ;"segment-related kludge"

<ROOM CROSSROADS
      (LOC ROOMS)
      (REGION "Along the GUH")
      (DESC "Crossroads")
      (LDESC
...
<ROOM TOLL-PLAZA
      ...
      (ACTION TOLL-PLAZA-F)
      (THINGS TOLL BOOTH TOLL-BOOTH-F)>
...
<ROUTINE FISHING-VILLAGE-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		...
		<COND (<NOT <FSET? ,FISHING-VILLAGE ,TOUCHBIT>>
		       ;<SETG INQ-NUMBER <TELL-RANDOM 100>>
		       ;<SETG NUMBER-ON-LINE <+ <TELL-RANDOM 3> 2>>
...
<ROUTINE I-INQ ("AUX" (NEW-PERSONS 0) (DOABLE <>))
	 ...
	 <COND (<AND <EQUAL? ,HERE ,FISHING-VILLAGE ,INQUISITION>
		     <G? .NEW-PERSONS 0>>
		<RETURN-FROM-MAP>
		...
		<COND (<EQUAL? .NEW-PERSONS 1>
		      ...
		      (T
		       <TELL "three people">)>
		<COND (<EQUAL? ,HERE ,FISHING-VILLAGE>
		       <TELL
", kicking and screaming, into the stone building." CR>
		       <RTRUE>)
		      (T
		       <TELL " in, who obediently take">
		       <COND (<EQUAL? .NEW-PERSONS 1>
			      <TELL "s a number">)
			     (T
			      <TELL " numbers">)>
...
adopted by the Fanucci Casino Rebuilding Act of 817." CR CR>
	 ...
	 <COND (<FSET? ,BROOM ,TRYTAKEBIT>
		...
		<FCLEAR ,NOT-HERE-OBJECT <+ 1 ,DRAW-CARDS-OFFSET>>
		;<PUT ,DRAW-CARDS-TABLE 1 1>
		<FCLEAR ,NOT-HERE-OBJECT <+ 2 ,DRAW-CARDS-OFFSET>>
		;<PUT ,DRAW-CARDS-TABLE 2 1>
		<FCLEAR ,NOT-HERE-OBJECT <+ 3 ,DRAW-CARDS-OFFSET>>
		;<PUT ,DRAW-CARDS-TABLE 3 1>
		<FCLEAR ,NOT-HERE-OBJECT <+ 4 ,DRAW-CARDS-OFFSET>>
...
<ROUTINE SETUP-CARDS ()
	 ...
	 <CLEAR ,S-FULL>
	 <PICK-RANK 0>
	 <PUTB ,F-CARD-TABLE 1 <RANDOM 15>>
	 <PICK-RANK 2>
	 <PUTB ,F-CARD-TABLE 3 <RANDOM 15>>
	 <PICK-RANK 4>
	 <PUTB ,F-CARD-TABLE 5 <RANDOM 15>>
	 <PICK-RANK 6>
	 <PUTB ,F-CARD-TABLE 7 <RANDOM 15>>
	 <PICK-RANK 8>
	 <PUTB ,F-CARD-TABLE 9 <RANDOM 15>>
	 <SETG J-SCORE 0>
	 <SETG YOUR-SCORE 0>
	 <SETG F-PLAYS 0>>
...
<ROUTINE SETUP-FANUCCI ("AUX" MARGIN TBL CARD-NUM-Y DISCARD-X CARD-NUM-1-X CARD-SPACE)
	 <COND (<APPLE?>
		<SETG MENU-SPACE 9>)
	       (<EQUAL? ,MACHINE ,IBM>
		<COND (<EQUAL? <LOWCORE HWRD> 640>
		       <SETG MENU-SPACE 14> ;"ega & cga")
		      (ELSE <SETG MENU-SPACE 12> ;"mcga")>)
	       (T
		<SETG MENU-SPACE 13>)>
	 <SCREEN ,S-FULL>
	 ...
	 <CURSET <GET ,PICINF-TBL 0>
		 <- <WINGET ,S-WINDOW ,WWIDE> <GET ,PICINF-TBL 1> <* 16 ,FONT-X>>>
	 ...
	 <COND (<APPLE?>
		<SET TBL ,F-PLAY-TABLE-APPLE>)
	       (T
		<SET TBL ,F-PLAY-TABLE>)>
	 <REPEAT ((C 0) Y (X <GET ,PICINF-TBL 1>))
	    <COND (<0? <MOD .C 3>>
		   <SET Y <GET ,PICINF-TBL 0>>
		   <COND (<NOT <0? .C>>
			  <SET X <+ .X <* ,FONT-X ,MENU-SPACE>>>)>)
		  (ELSE <SET Y <+ .Y ,FONT-Y>>)>
	    <CURSET .Y .X>
	    <TELL " " <GET .TBL .C>>
	    <COND (<G? <SET C <+ .C 1>> 14> <RETURN>)>>
	 <SCREEN ,S-TEXT>
	 <CLEAR ,S-TEXT>>
...
<ROUTINE DRAW-CARDS ("AUX" X (CNT 0))
	 <REPEAT ()
		 ...
		 <COND (<EQUAL? .CNT 5>
			<RETURN>)
		       (<NOT <FSET? ,NOT-HERE-OBJECT
				    <+ .CNT ,DRAW-CARDS-OFFSET>>>
			;<EQUAL? <GET ,DRAW-CARDS-TABLE .CNT> 1>
			<FSET ,NOT-HERE-OBJECT <+ .CNT ,DRAW-CARDS-OFFSET>>
...
<ROUTINE PICK-RANK (NUM)
	 <PUTB ,F-CARD-TABLE .NUM <COND (<PROB ,PICTURE-CARD-PROB>
					 <+ 11 <RANDOM 9>>)
					(T
					 <RANDOM 11>)>>>
...
<ROUTINE DRAW-CARD (X RANK SUIT "AUX" Y PIC)
	 <SET RANK <GETB ,F-CARD-TABLE .RANK>>
         <SET SUIT <GETB ,F-CARD-TABLE .SUIT>>
	 <SET Y <GET ,PICINF-TBL 0>>
         <COND (<EQUAL? .RANK 0>
...
<ROUTINE UPDATE-SCORES ()
	 ...
	 <TELL N ,J-SCORE>
	 <CURSET <GET ,PICINF-TBL 0>
		 <- <WINGET ,S-WINDOW ,WWIDE>
		    <GET ,PICINF-TBL 1>
		    <* 4 ,FONT-X>>>
	 ;<PICINF-PLUS-ONE ,YOUR-SCORE-LOC>
	 ;<CURSET <GET ,PICINF-TBL 0>
		  <+ <GET ,PICINF-TBL 1> <* ,FONT-X 12>>>
		 ...
		 <COND (<EQUAL? .KEY ,CLICK1 ,CLICK2>
			...
			<PICINF-PLUS-ONE ,F-MENU-LOC>
	 		...
	 		<REPEAT ()
			     ...
			     <SET BR-X <+ <* ,FONT-X ,MENU-SPACE> .TL-X>>
			     <REPEAT ()
...
double-click on it to select that play.">
			...
			<BOLD-MOVE .X .Y .PTR>)
		       (<OR <EQUAL? .KEY ,RIGHT-ARROW>
			    <EQUAL? .KEY !\R !\r 32 ;"space bar">>
			<UNBOLD-MOVE .X .Y .PTR>
			<COND (<EQUAL? .X
				       <+ .LEFT <* ,FONT-X <* 4 ,MENU-SPACE>>>>
			       <PICINF-PLUS-ONE ,F-MENU-LOC>
	 		       <SET X <GET ,PICINF-TBL 1>>
			       <SET PTR <- .PTR 12>>)
			      (T
...
<ROUTINE PLAY-SELECTED (PTR "AUX" CNT X NUM)
	 <COND (<EQUAL? .PTR 14> ;"resign"
		...
		<SET CNT 0>
		<REPEAT ()
			<COND (<EQUAL? <GETB ,F-CARD-TABLE .CNT> 0>
			       <FCLEAR ,NOT-HERE-OBJECT
				       <+ </ .CNT 2> ,DRAW-CARDS-OFFSET>>
...
"The jester snickers. \"You've no empty slot; you forfeit your shot!\"">
			       ...
			       <RETURN>)>
			<SET CNT <+ .CNT 2>>>)
	       (<AND <EQUAL? <GETB ,F-CARD-TABLE 2> 0>
		     <EQUAL? <GETB ,F-CARD-TABLE 4> 0>
		     <EQUAL? <GETB ,F-CARD-TABLE 6> 0>
		     <EQUAL? <GETB ,F-CARD-TABLE 8> 0>>
		<CLEAR ,S-TEXT>
		<SOUND 1>
		<TELL
...
" just now; you have no drawn cards!">
		...
		<COND (<OR <APPLE?>
		       ...
		       <TELL "/joystick">)>
		<TELL
" or type the number of the card you'd like to " <GET ,F-PLAY-TABLE-LC .PTR> ".">
		<2CR-TO-PRINTER>
		<REPEAT ()
		       <SET X <INPUT 1>>
		       <MOUSE-INPUT?>
		       ...
		       <COND (<OR <AND <EQUAL? .X !\1>
			      ...
			      <SOUND 1>
			      <TELL
"You can't " <GET ,F-PLAY-TABLE-LC .PTR> " that card; it hasn't been drawn yet!">
			      <2CR-TO-PRINTER>)
			     (<EQUAL? .X !\1>
			      <SET X 2>
			      <RETURN>)
			     ...
			     (T
			      ...
			      <2CR-TO-PRINTER>)>>
		<SET NUM <MOD <* <GETB ,F-CARD-TABLE .X>
				 <GETB ,F-CARD-TABLE <+ .X 1>>
				 .PTR> 37>>
		<COND (<EQUAL? .PTR 5> ;"undertrump"
		       <COND (<G? ,F-WIN-COUNT 0>
			      <SETG F-WIN-COUNT <+ ,F-WIN-COUNT 1>>
			      <COND (<EQUAL? ,F-WIN-COUNT 3>
				     <FCLEAR ,NOT-HERE-OBJECT
					     <+ </ .X 2> ,DRAW-CARDS-OFFSET>>
				     ;<PUT ,DRAW-CARDS-TABLE </ .X 2> 1>
				     <PUTB ,F-CARD-TABLE .X 0>
				     <PUTB ,F-CARD-TABLE <+ .X 1> 0>
				     ;"go straight to win-text"
				     <RFALSE>)>)
			     (<AND <EQUAL? <GET ,J-LAST-CARD 0> ,TREBLED-RANK>
				   <EQUAL? <GET ,J-LAST-CARD 1> ,FROMP-SUIT>>
			      <SETG F-WIN-COUNT 1>)>)
		      (T
		       <SETG F-WIN-COUNT 0>)>
		<COND (<EQUAL? <GET ,F-SCORES .NUM> 0>
		       ...
		       <TELL
"   The jester is outraged! \"You can't " <GET ,F-PLAY-TABLE-LC .PTR> " ">
		       <PRINT-CARD-NAME .X>
		       <COND (<PROB 6>
			      <TELL " when ">
			      <PRINT-CARD-NAME 0>
		      ...
		      (T
		       ...
		       <COND (<EQUAL? .PTR 1> ;"move card to discard pile"
			      ...
			      <PUTB ,F-CARD-TABLE 1
				   <GETB ,F-CARD-TABLE <+ .X 1>>>)>
		       <FCLEAR ,NOT-HERE-OBJECT <+ </ .X 2>,DRAW-CARDS-OFFSET>>
		       ;<PUT ,DRAW-CARDS-TABLE </ .X 2> 1>
		       <PUTB ,F-CARD-TABLE .X 0>
		       <PUTB ,F-CARD-TABLE <+ .X 1> 0>)>)>
	 <RFALSE>>
...
<ROUTINE PRINT-CARD-NAME (CNT)
	 <TELL <GET ,F-RANK-TABLE <GETB ,F-CARD-TABLE .CNT>>>
	 <COND (<L? <GETB ,F-CARD-TABLE .CNT> 12>
		<TELL " " <GET ,F-SUIT-TABLE
			   <GETB ,F-CARD-TABLE <+ .CNT 1>>>>)>>
...
"before a New Sun!\""
"from Second Seat in a two-person game!\""
"cries, \"Daring move!\""
...
at the latter end, and passes a large, eye-catching sign.")
      ...
      (ACTION WARNING-ROOM-F)>

<ROUTINE WARNING-ROOM-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,FOOZLE-SEGMENT-PIC ,PICINF-TBL>
		<RFALSE>)
	       (<AND <EQUAL? .RARG ,M-BEG>
...
2. The ">
		<COND (<PRSO? ,VERITASSI-DOOR>
		       <TELL D ,PREVARICON-DOOR>
		       <PRINT " does not lead to ">
		       <TELL "Wishyfoo">)
		      (<PRSO? ,PREVARICON-DOOR>
		       <TELL D ,WISHYFOO-DOOR>
		       <PRINT " does not lead to ">
		       <TELL "Wishyfoo">)
		      (T
		       <TELL D ,VERITASSI-DOOR>
		       <PRINT " does not lead to ">
		       <TELL "Veritassi">)>
		<TELL " territory.\"" CR>)
	       (<VERB? OPEN ENTER>
