...
<ROUTINE LAKE-SEGMENT-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,LAKE-SEGMENT-PIC ,PICINF-TBL>
...
"The royal yacht sits by the dock, bobbing gently in the swell of the lake.")
	...
	(ACTION YACHT-F)
	(THINGS ( SMALL BRASS) PLAQUE YACHT-PLAQUE-F
		RED BUTTON YACHT-BUTTON-F
	 	BLUE BUTTON YACHT-BUTTON-F
		GREEN BUTTON YACHT-BUTTON-F
		YELLOW BUTTON YACHT-BUTTON-F
		( WHITE MIDDLE CENTER FIFTH) BUTTON YACHT-BUTTON-F)>
...
<ROUTINE YACHT-PLAQUE-F ()
	<COND (<VERB? EXAMINE READ>
	       <TELL
"\"Made by the Frobozz Magic Royal Yacht Company.\"" CR>)>>
...
seaman; but the truth's not so queer -- you need nautical gear!\"">
		       <J-EXITS>)
		      (<OR <ADJ-USED? ,PSEUDO-OBJECT ,W?WHITE ,W?FIFTH>
			   <ADJ-USED? ,PSEUDO-OBJECT ,W?MIDDLE ,W?CENTER>>
		       <COND (<EQUAL? ,HERE ,LAKE-FLATHEAD>
			      <DEQUEUE I-YACHT>
			      <TELL ,NOTHING-HAPPENS>)
			     (T
...
Beautiful freshwater fish swim among slowly waving spenseweeds.")
      ...
      (THINGS ( BEAUTIFUL FRESHWATER) FISH LAKE-BOTTOM-FISH-F)>

<ROUTINE LAKE-BOTTOM-FISH-F ()
	 <COND (<TOUCHING? ,PSEUDO-OBJECT ;,LAKE-BOTTOM-FISH>
		<CANT-REACH ,PSEUDO-OBJECT ;,LAKE-BOTTOM-FISH>)>>
...
<ROUTINE LOWEST-HALL-ENTER-F
	 ...
	 <COND (.RARG
	        <RETURN ,LOWEST-HALL>)>
	 <RETURN-FROM-MAP>
	 <PRINT "You lose your footing on the ">
	 <TELL "gravel, drop your possessions, and begin sliding
...
<ROUTINE LAIR-EXIT-F ("OPTIONAL" (RARG <>)) ;"called by NEXT-ROOM?"
	 <COND (<OR .RARG
		...
		<RETURN ,G-U-WOODS>)>
	 <PRINT "You lose your footing on the ">
	 <TELL "treacherous path, and tumble painfully down
a steep incline. ">
...
"\"One of the many awe-inspiring features of Dimwit's castle in Flatheadia.\"")
      ...
      (THINGS ( TALL WIDE PINE OAK) ( OAK PINE TREE) TREE-PS)
      (ACTION G-U-WOODS-F)>

<ROUTINE G-U-WOODS-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,LAKE-SEGMENT-PIC ,PICINF-TBL>
		<COND (<NOT <FSET? ,G-U-WOODS ,TOUCHBIT>>
		       <QUEUE I-FOX -1>)>
		<RFALSE>)>>
...
"You trudge along beneath the searing gaze of an artificial desert sun">
		       <COND (<EQUAL? ,DESERT-DEATH 4>
			     ...
			     (<EQUAL? ,DESERT-DEATH 3>
			      <TELL ". You won't last much longer in this heat">)>
		       <TELL ,ELLIPSIS>)>
		<RFALSE>)
	       (<AND <EQUAL? .RARG ,M-END>
...
<ROUTINE CONDUCTOR-PIT-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		<PICINF ,LAKE-SEGMENT-PIC ,PICINF-TBL>
		<COND (<NOT <IN? ,CONDUCTOR-STAND ,CONDUCTOR-PIT>>
