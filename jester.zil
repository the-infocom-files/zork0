...
pirates? In that case, I hope you fall into a bottomless pit.\"">)>
	 ...
	 <COND (<IN? ,JESTER ,HERE> ;"example: going west from West Wing twice"
		...
		<PRINT "You suddenly ">
		<TELL
"notice another " D ,TURNED-INTO ", wearing a jester's costume.
...
wearing sunglasses and lots of tanning lotion.">)
		...
		<FSET ,JESTER ,TOUCHBIT>
		<PRINT "You suddenly ">
		<TELL "notice a wiry fellow with an outrageous costume and a
...
"See you in the funny papers!\" A breeze begins picking up." CR>)
	       (<AND <PROB 15>
		     ...
		     <NOT <EQUAL? ,HERE ,LAKE-FLATHEAD ,UNDERWATER ,LAKE-BOTTOM
				  	,HOLD ,GONDOLA ,BATS-LAIR ,ROOF ,PARAPET
					,CONDUCTOR-PIT ,OUTER-BAILEY>>
...
jester any fish." CR>)
		...
		<QUEUE I-SUFFOCATE -1>
		<SETG P-CONT -1> ;<RFATAL>
		<TELL "Catch you aroun', clown!\" ">
		<PRINT "You suddenly ">
		<TELL "find yourself having a bit of
...
adventurer look under the ">
		<PRINTB ,HOLEY-SLAB>
		<TELL " slab in the Crypt." CR>)
	       (<VERB? FOLLOW>
...
onto the slate." CR>)
		...
		<COND (<EQUAL? ,STUMP-X 0>
		       ;<SETG STUMP-X <TELL-RANDOM 3>>
		       <SETG STUMP-X <RANDOM 3>>
		       <COND (<PROB 50>
			      <SETG STUMP-X <* ,STUMP-X -1>>)>
		       ;<SETG STUMP-Y <TELL-RANDOM 3>>
...
<ROUTINE WEST-SEGMENT-F ("OPT" (RARG <>))
...
you want to move it to: L, C, or R. ">
		<COND (,ACTIVE-MOUSE
		       <TELL "Or, if you'd rather use your mouse">
		       <COND (<OR <APPLE?>
				  <EQUAL? <LOWCORE INTID> ,IBM>>
			      <TELL "/joystick">)>
		       <TELL
", click on the weight you want to move, then click on the peg you want
to move it to. ">)>
		<TELL "You can Undo your move by typing U">
		<COND (,ACTIVE-MOUSE
		       <TELL " (or clicking on the UNDO spot)">)>
		<COND (<NOT <FLAG-ON? ,F-UNDO>> ;"system has no UNDO"
		       <TELL
...
"   You hear a \"click\" from the direction of the Tower">
			...
			<TELL ". The small louvered door swings silently shut.">
			<2CR-TO-PRINTER>
			<RETURN>)>
		 <COND (,DEMO-VERSION?
...
"You shut the louvered door, concealing the Tower of Bozbar from view.">
			...
			<COND (<NOT ,TOWER-CHANGED>
			       <TELL "You can't undo until you make a move!">
			       <2CR-TO-PRINTER>)
			      (.JUST-UNDID
			       <TELL
"You just undid; you can't undo again until you move again!">
			...
			<COND (<NOT <EQUAL? <LOC .WGT>
			       ...
			       <TELL "There's no " D .WGT " anymore -- you crushed it.">
			       <2CR-TO-PRINTER>)
			      (T
			       <COND (,ACTIVE-MOUSE
				      <TELL "Use your mouse">
...
<ROUTINE WEIRD-F ("OPT" (RARG <>))
	 <COND (<EQUAL? .RARG ,M-ENTER>
		...
		<COND (<AND <NOUN-USED? ,LEFT-RIGHT ,W?LEFT>
			    <EQUAL? ,TOWER-BEATEN ,PYRAMID-L>>
		       <DO-WALK ,P?WEST>)
		      (<AND <NOUN-USED? ,LEFT-RIGHT ,W?RIGHT>
			    <EQUAL? ,TOWER-BEATEN ,PYRAMID-R>>
		       <DO-WALK ,P?EAST>)>)
	       (<EQUAL? .RARG ,M-LOOK>
		<TELL
...
<ROUTINE PLANT-STUNNED ("OPTIONAL" (OBJ <>))
	 <COND (<NOT .OBJ>
		<SET OBJ ,WINNER>)>
	 <TELL
"There is no response -- perhaps" T .OBJ " is simply stunned to hear
